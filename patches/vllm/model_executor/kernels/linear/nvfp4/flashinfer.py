...
class FlashInferCutlassNvFp4LinearKernel(NvFp4LinearKernel):
    ...
    def is_supported(
        ...
        if (
            ...
            # (RTX 5090 / RTX PRO 6000, capability 12.x). FlashInfer's NVFP4
            # mm_fp4 GEMM is broken on sm_120 (all FlashInfer backends fail:
            # flashinfer-ai/flashinfer#2577). vLLM ships a dedicated
            # nvfp4_scaled_mm_sm120 CUTLASS kernel, so fall through to the
            # next registry entry (CutlassNvFp4LinearKernel) on sm_120.
