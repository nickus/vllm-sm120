...
class FlashInferExperts(mk.FusedMoEExpertsModular):
    ...
    def _supports_quant_scheme(
        ...
        return (
            ...
            # [vllm-sm120 overlay] FlashInfer's sm_120 NVFP4 fused-MoE path is
            # unreliable on capability 12.x (garbage output / illegal
            # instruction / CUDA-graph-replay crashes:
            # flashinfer-ai/flashinfer#2577, NVIDIA/cutlass#3096). Decline
            # sm_120 here so the NvFp4 MoE oracle falls through to vLLM's
            # native CUTLASS blockwise backend (VLLM_CUTLASS / CutlassExpertsFp4,
            # nvfp4_blockwise_moe_kernel.cu), which is the intended sm_120 path.
            or (
